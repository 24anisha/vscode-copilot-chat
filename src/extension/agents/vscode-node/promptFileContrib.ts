	...
	constructor(
		...
		if ('registerCustomAgentProvider' in vscode.chat) {
			...
			const updateExploreProvider = () => {
				const isExploreAgentEnabled = configurationService.getExperimentBasedConfig(ConfigKey.ExploreAgentEnabled, experimentationService);
			...
			this._register(configurationService.onDidChangeConfiguration(e => {
				if (e.affectsConfiguration(ConfigKey.ExploreAgentEnabled)) {
					updateExploreProvider();
				}
			}));

			if ('onDidTreatmentsChanged' in experimentationService) {
				this._register(experimentationService.onDidTreatmentsChanged(() => {
					updateExploreProvider();
				}));
			}
		}
